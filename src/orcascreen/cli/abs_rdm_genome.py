...
def rdm_genome_alt(length: int, logfile: str = None):
    """
    ...
    if logfile is not None :
        with open(logfile, "w") as flog :
            flog.write(f"Randomly chosen domain boundaries (({len(domain_bnd)})) :\n{domain_bnd }\n")
            flog.write(f"Chosen bases per domain ({len(info_bases)}) :\n{info_bases}\n")
            flog.write(f"Chosen bases per palindromic domain ({len(info_pal)}) :\n{info_pal}\n")            

    return genome


def random_genome(length: int, mut_path: str):
    """
    Generate a random genome and save it as a fasta file.

    Parameters
    ----------
        length (int) : Length of the genome to generate.
        mut_path (str) : Path to the output directory.

    Outputs
    -------
    - A fasta file containing the random genome sequence.
    - A log file summarizing the generation process.

    Notes
    -----
        The generated genome will have a single chromosome named 'fake_chr'.
    """
    ...
    logfile = f"{mut_path}/random_genome.log"
    genome = rdm_genome_alt(length=length, logfile=logfile)
    ...
    log_path = f"{output_path}/genome_resume.log"
