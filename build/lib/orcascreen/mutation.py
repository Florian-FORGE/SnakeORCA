...
from orcascreen.config_mut import config_data
...
class Mutator():
    ...
    def permutations_intra(self, mutation: Mutation, binsize: int = None):
        ...
        """
        binsize = config_data["binsize_permut"] if binsize is None else binsize
        subseq = self.get_ref(mutation)
    ...
    def mutate(self, binsize_permut: int = None):
        """
        ...
        if all(mutation.op == "permutations_inter" for mutation in self.mutations) :
            ...
            for mutation in mutations:
                ...
                if mutation.op == "shuffle":
                    ...
                    self.permutations_intra(mutation, binsize=binsize_permut)
                else:
                    self.chromosome_mutations[mutation.chrom] -= 1
