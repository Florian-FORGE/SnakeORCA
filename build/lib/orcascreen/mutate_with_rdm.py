...
from orcascreen.mutation import Mutation, Mutator
...
def generate_random_pos_mutations(mutations: List[Mutation], 
    ...
    if distrib == "rotate_bins" or distrib == "shuffle_bins" :
        ...
        binsize_distrib = 128_000 if binsize_distrib is None else binsize_distrib
        
        partition = range(region[1], region[2], binsize_distrib)
        mut_per_bin = [[[], [False, 0], [False, 0]] for i in partition]
        
        if any([mut.end - mut.start > binsize_distrib for mut in mutations]) :
            logging.info(f"{distrib} distribution cannot be used due to mutation size being "
                         f"larger than binsize_distrib ({binsize_distrib}). Proceeding without "
        ...
        for mut in mutations:
            ...
            for i, bin_start in enumerate(partition):
                ...
                if (
                    (((new_mut.end - binsize_distrib <= 0) and (new_mut.end - half_len >= 0)) 
                     and 
                     ((new_mut.start >= 0) and (new_mut.start +  half_len <= binsize_distrib)))
                    ) :
                    mut_per_bin[i][0].append(new_mut)
                    break  # Stop after assigning to the first matching bin (all the mutation is inside the bin)

                elif ((new_mut.end - binsize_distrib <= 0) and (new_mut.end - half_len >= 0)) :
                    mut_per_bin[i][0].append(new_mut)
                ...
                elif ((new_mut.start >= 0) and (new_mut.start +  half_len <= binsize_distrib)) :
                    mut_per_bin[i][0].append(new_mut)
                    mut_per_bin[i][2] = [True, new_mut.end - binsize_distrib]
                    break  # Stop after assigning to the first matching bin (end of mutation is after the bin)
        

        def is_valid_permutation(mut_per_bin: list, binsize_distrib: int):
            n = len(mut_per_bin)
            ...
            for i in range(n - 1):
                ...
                if next_val[1][0]:
                    ...
                    if (max_end is not None) and (max_end >= binsize_distrib + next_val[1][1]):
                        # Note: binsize_distrib + next_val[1][1] because next_val[1][1] is the offset from the next bin start (<0)
                        print(f"Denied permutation because invalid adjacent bins (next overstepping): {max_end}, {binsize_distrib + next_val[1][1]}")
                        return False
                ...
                if len(curr_val[0]) > 0 :
                    for j in range(len(curr_val[0])) :
                        if j == len(curr_val[0])-1  and len(next_val[0]) > 0 : 
                            if curr_val[0][j].end > binsize_distrib + next_val[0][0].start :
                                # Note: binsize_distrib + next_val[0][0].start because next_val[0][0].start is the offset from the next bin start
                                print(f"Denied permutation after last check (overstepping bin): {curr_val[0][j].end} > {binsize_distrib + next_val[0][0].start}")
                                return False
        ...
        if distrib == "rotate_bins":
            def rotate_bins(mut_per_bin : list, binsize_distrib: int, max_attempts=100000):
                ...
                for i in range(1, max_attempts):
                    ...
                    if is_valid_permutation(new_mut_per_bin, binsize_distrib) and new_mut_per_bin != orig:
                        return new_mut_per_bin
                    elif is_valid_permutation(new_mut_per_bin, binsize_distrib) :
            ...
            new_mut_per_bin = rotate_bins(mut_per_bin, binsize_distrib)
                
        else :
            def shuffle_bins(mut_per_bin : list, binsize_distrib: int, max_attempts=1000000):
                orig = copy.deepcopy(mut_per_bin)
                for i in range(1, max_attempts):
                    rdm.seed(rdm_seed*i)
                    rdm.shuffle(mut_per_bin)
                    if is_valid_permutation(mut_per_bin, binsize_distrib) and mut_per_bin != orig:
                        return mut_per_bin
                    elif is_valid_permutation(mut_per_bin, binsize_distrib) :
                        print(f"Found a valid permutation after {i} attempts.")
                        if mut_per_bin == orig:
                            print(f"Sadly, the permutation is the same as the original one after {i} attempts.")
            ...
            new_mut_per_bin = shuffle_bins(mut_per_bin, binsize_distrib)

        intervals = []
        
        for i, bin in enumerate(new_mut_per_bin):
            for mut in bin[0]:
                mut.start += i * binsize_distrib
                mut.end += i * binsize_distrib
...
def main(mutationfile, bed, genome, path: str, mutationtype: str, 
         binsize_permut: int = 128_000, nb_random: int = 0, 
         rdm_pos: bool = True, excluded_domains: str = None, 
         included_into: str = None, distrib: str = "rotate_bins", 
         binsize_distrib: int = 128_000, region: str = ["chr9", 0, 31_999_999], 
         order: list = None) :
    """
    ...
    for i in range(nb_random):
        ...
        if rdm_pos:
            random_mutations = generate_random_pos_mutations(mutations=mutations, genome=genome, rdm_seed=rdm_seed, 
                                                             ...
                                                             included_into=included_into, distrib=distrib, binsize_distrib=binsize_distrib, 
                                                             region=region)
        else :
    ...
    for name, mutator in mutators.items() :
        mutator.mutate(binsize_permut=binsize_permut)
...
def parse_arguments():
    ...
    parser.add_argument("--binsize_permut",
                        required=False, help="In case the choosen mutation type is 'permutations_intra', the binsize is used to create a partition. Defaults to 128_000.")
    ...
    parser.add_argument("--binsize_distrib",
...
if __name__ == '__main__':
    ...
    binsize_distrib = int(args.binsize_distrib) if args.binsize_distrib is not None else None
    binsize_permut = int(args.binsize_permut) if args.binsize_permut is not None else None
    region = None
    ...
    main(mutationfile=args.mutationfile, 
         ...
         binsize_distrib=binsize_distrib, 
         region=region,
         order=order)
