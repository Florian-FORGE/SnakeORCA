...
from plotnine import ggplot, aes, geom_boxplot, geom_jitter, scale_fill_brewer, theme_minimal, theme, xlab, ylab, element_text
import re
...
def boxplot_slide(descrip: str, data_file: str, analysis_path:str, output_file: str, 
    ...
    with PdfPages(plots_path, keep_empty=False) as pdf:
        ...
        for i, score in enumerate(score_types) :
            ...
            plot = (
                    ggplot(_data, aes(x='hue_name', y='values', fill='hue_name')) +
                    geom_boxplot() +
                    geom_jitter(color="black", size=0.4, alpha=0.9) +
                    scale_fill_brewer(type='qual', palette='Set2') +
                    theme_minimal() +
                    theme(figure_size=(60, 12.75), legend_position='none')  +
                    xlab("") +
                    ylab(f"{score} deviation\n")
                    )
            
            plot += theme(
                            axis_text_x=element_text(size=64, weight="bold"),       # X-axis tick labels
                            axis_text_y=element_text(size=72, weight="bold"),       # Y-axis tick labels
                            axis_title_y=element_text(size=80, weight="bold"),      # Y-axis title
                            axis_title_x=element_text(size=80, weight="bold"),      # X-axis title (if present)
                            plot_margin=-0.01
                            )
